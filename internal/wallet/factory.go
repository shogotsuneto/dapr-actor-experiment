// Package wallet provides primitives for OpenAPI-based schema validation.
//
// Code generated from OpenAPI specification. DO NOT EDIT manually.
package wallet

import (
	"fmt"
	"github.com/dapr/go-sdk/actor"
)

// NewActorFactory creates a factory function for Wallet with a cleaner API.
// Returns a factory function compatible with Dapr's RegisterActorImplFactoryContext.
// Usage: s.RegisterActorImplFactoryContext(wallet.NewActorFactory())
func NewActorFactory() func() actor.ServerContext {
	return func() actor.ServerContext {
		// Create a new Wallet instance using our custom constructor
		impl := NewWallet()
		
		// Compile-time check ensures the implementation satisfies the schema
		var _ WalletAPI = impl
		
		// Verify the actor type matches the schema
		if impl.Type() != ActorTypeWallet {
			panic(fmt.Sprintf("actor implementation Type() returns '%s', expected '%s'", impl.Type(), ActorTypeWallet))
		}
		
		return impl
	}
}