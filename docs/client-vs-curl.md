...
2. **Curl Commands** (`scripts/quick-demo.sh`, `scripts/test-counter-actor.sh`, `scripts/test-bank-account-actor.sh`) - Raw HTTP calls
...
```bash
# scripts/quick-demo.sh (and individual test scripts)
curl -X POST \
