...
This document explains the BankAccount transactions projection feature that demonstrates CQRS (Command Query Responsibility Segregation) patterns using event sourcing.
...
JWT `sub` claim is mapped to `user_id` parameter for owner filtering.
...
```json
{
  "queries": ["my_transactions", "my_account_balance", "my_transaction_summary"]
}
```
...
- **JWT Authentication**: Query endpoints require valid JWT tokens with `user_id` mapping from JWT `sub` claim
